...
{
    class UTSCSVFileParser: ICSVFileParser
    {
        private volatile int LineNumFromScanInfoFile = 0;
        private int LineNumReadout = 0;
        private static UTSCSVFileParser uniqueInstance;
        ...
        /// </summary>
        /// <returns></returns>
        public static UTSCSVFileParser GetInstance()
        {
            lock (locker)
            {
                if (uniqueInstance == null)
                {
                    uniqueInstance = new UTSCSVFileParser();
                }
            }
        ...
        public TestCamLog ReadLastRecorodFromCSVFile(string fileName)
        {
            TestCamLog lastRecord = null;
            int s = fileName.LastIndexOf("\\");
            int e = fileName.LastIndexOf(".");
            String trimFileName = fileName.Substring(s+1,e-s-1);
            LineNumFromScanInfoFile = uj.ReadLastOperLine(trimFileName);

            string stationIndexStr = TestContext.Stationindex;
            try
            {
                StationIndex = Int32.Parse(stationIndexStr);
            }
            ...
            this.WorkID = TestContext.Workid;
            IsFinalStation = TestContext.Islaststation;
            ...
            try
            {
                ...
                if (logRecords != null && logRecords.Length>0)
                    lastRecord = logRecords[logRecords.Length - 1];
                if(LineNumReadout>0)
                    uj.WriteLastOperLine(trimFileName, LineNumReadout);
            }
            catch (Exception ex)
            {
                string ex_str = ex.ToString();
                m_log.Error("ReadCSVFile " + ex_str);
            }
        ...
        [MethodImpl(MethodImplOptions.Synchronized)]
        public bool UpdateDB(TestCamLog currTestLog,ProductTestInfo lastTinfo, ProductDashboard dashboard)
        {
            bool isInsertSucc1 = true;
            if (currTestLog == null)
                return false;
            ProductTestInfo currTinfo = GenerateTestInfo(currTestLog);
            try
            {
                ...
                if (isTotallyNew)  // new UID(None is retrieved)
                {
                    ...
                    if (result == PASS)
                    {
                        ...
                        {
                            // TO DO F2DB
                            currTinfo.PlatForm = "UTS";
                            currTinfo.StationIndex = TestContext.Stationindex;
                    ...
                    if (isTotallyNew)  // new UID(None is retrieved)
                    {
                        ...
                        if (result == PASS)
                        {
                            if (dashboard == null)
                            {
                                ...
                                DBTool.InsertProductTestInfo(currTinfo);
                            }
                            else
                            {
                                int i = GetInput(dashboard);
        ...
        public ProductTestInfo GenerateTestInfo(TestCamLog logObj)
        {
            if (logObj == null)
                return null;
            ...
            testInfo.time = TestContext.DUTTestTime;
            testInfo.pcmac = this.PCmac;
            //testInfo.station = TestContext.Product+"@"+TestContext.Stationname ;
            testInfo.station = TestContext.Stationname;
            testInfo.work_id = TestContext.Workid;
            testInfo.product = TestContext.Product;
            testInfo.frequency = 1;
            testInfo.rework = 0;
            testInfo.errorinfo = logObj.Comment;
            testInfo.process = 0;  
            testInfo.failtimes = (logObj.Status == "P") ? 0 : 1;
            testInfo.IsLastStation = TestContext.Islaststation ? "1" : "0";
        ...
        {
            if (e.LineNumber <= LineNumFromScanInfoFile)
            {
        ...
        private void AfterEvent(EngineBase engine, FileHelpers.Events.AfterReadEventArgs<TestCamLog> e)
        {
            ...
            string line = e.RecordLine;

            if (e.Record.Status != "P" && e.Record.Status != "F")
            {
                e.SkipThisRecord = true;
            }
            else
            {
                e.Record.SerialNumber = TestContext.UID;
                
            } 
        }

        public int GetUIDPosi(string line,string UIDColumnNameInLog)
        {
            int posi = -1;
            string[] tokens = line.Split(',');
            for(int i=0;i<tokens.Length;i++)
            {
                if(tokens[i]== UIDColumnNameInLog)
                {
                    posi = i;
                    break;
                }
        ...
        public string GetUIDStr(string line,int posi)
        {
            string[] tokens = line.Split(',');
            if (tokens.Length - 1 < posi)
                return "";
            return tokens[posi];
        }
    }

...
namespace LogParserAndTransfer
{
    ...
    public class KineticEISControlFileMonitor : IDirectoryMonitor
    {
        ...
        public KineticEISControlFileMonitor(string dirPath,string filter, IniFileChangeListener changeListener)
        {
        ...
        private void OnChange(object sender, FileSystemEventArgs e)
        {
            ...
            m_log.Info("FileSystemEvent " + fTypeStr+","+ fullPath);
            // can not find EIS_Control.txt file delete/create event,but create==change event
            if (fTypeStr == "Deleted") 
            {
                return;
            }
            ...
            else
            {
                ...
                {
                    lastEventTime = DateTime.Now;
                    m_log.Info("FileSystemEvent Interal<2s,not call ActionFileChanged()");
                    return;
                }
                lastEventTime = DateTime.Now;
            }
